...
_CATALOG_ENTRIES = (
    ...
    },
    {
        ...
        "argv_pattern": "typetreeflow selection-review strategy --outdir <verify-genus-run> [--bounded-smoke-outdir <dir>]",
        "json_stdout": True,
        "write_behavior": "none",
        "requires_outdir": True,
...
_SERVER_VALIDATION_RESULT_REVIEW_QUEUE_SUMMARY_FIELDS: list[str] = [
    ...
    "output_path",
    "output_written",
    "triage_queue_recommended_request",
    "triage_queue_recommended_request_target",
    "triage_queue_recommended_next_command",
    "diagnostic_count",
    "dry_run",
    "writes_outputs",
    "writes_workflow_outputs",
...
_SERVER_VALIDATION_RESULT_QUALITY_REVIEW_TEMPLATE_SUMMARY_FIELDS: list[str] = [
    ...
    "output_path",
    "output_written",
    "quality_review_recommended_request",
    "quality_review_recommended_request_target",
    "quality_review_recommended_next_command",
    "template_only",
    "completed_review",
...
_SELECTION_REVIEW_STRATEGY_SUMMARY_FIELDS: list[str] = [
    ...
    "selected_datasets_command_preview_truncated",
    "selected_datasets_command_preview_only",
    "recommended_request_target",
    "recommended_request",
    "recommended_next_command",
    "review_artifacts",
...
_DOWNLOAD_SMOKE_INSPECTION_SUMMARY_FIELDS: list[str] = [
    ...
    "recommended_quality_gate_next_command",
...
_PARAMETER_CATALOG: dict[tuple[str, str | None], list[dict[str, object]]] = {
    ...
    ("download-smoke", "prepare"): [
        ...
        {
            "name": "--inspection-quality-profile",
            "kind": "choice",
            "required": False,
            "repeatable": False,
            "purpose": "recommended inspect quality profile; fragmentation blocks fragmented FASTA and WGS/scaffold/contig header keywords",
        },
        {
    ...
    ("download-smoke", "inspect"): [
        ...
        },
        {
            ...
            "required": False,
            "repeatable": False,
            "purpose": "optional local quality profile; fragmentation blocks fragmented FASTA and WGS/scaffold/contig header keywords",
        },
        {
...
def _render_target_argv(request: dict[str, object]) -> list[str]:
    ...
    if command == "download-smoke":
        if subcommand == "prepare":
            ...
            inspection_max_ambiguous = _optional_int(
                request,
                "inspection_max_fasta_ambiguous_bases",
            )
            if inspection_max_ambiguous is not None:
                argv.extend(
                    [
                        "--inspection-max-fasta-ambiguous-bases",
                        str(inspection_max_ambiguous),
                    ]
                )
            ...
            inspection_quality_profile = _optional_string(
                request,
                "inspection_quality_profile",
            )
            if inspection_quality_profile:
                argv.extend(
                    [
        ...
        if subcommand == "inspect":
            ...
            max_ambiguous = _optional_int(request, "max_fasta_ambiguous_bases")
            if max_ambiguous is not None:
                argv.extend(["--max-fasta-ambiguous-bases", str(max_ambiguous)])
            ...
            quality_profile = _optional_string(request, "quality_profile")
            if quality_profile:
                argv.extend(["--quality-profile", quality_profile])
