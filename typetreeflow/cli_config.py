...
def build_app_config_from_args(
    ...
    *,
    ...
    if (
        args.provider_handoff_dir is not None
        and not args.report_only
        and not package_results_command
    ):
        raise ValueError(
            "--provider-handoff-dir is only supported with --report-only "
            "or package-results."
        )
    if (
        args.provider_request_dir is not None
        and not args.report_only
        and not package_results_command
    ):
        raise ValueError(
            "--provider-request-dir is only supported with --report-only "
            "or package-results."
        )
    if (
    ...
    if (
        args.provider_request_external_genomes_dir is not None
        and not args.report_only
        and not package_results_command
    ):
        raise ValueError(
            "--provider-request-external-genomes-dir is only supported with "
            "--report-only or package-results."
        )
    if (
        args.external_genomes_install_plan_dir is not None
        and not args.report_only
        and not package_results_command
    ):
        raise ValueError(
    ...
    if (
        args.download_smoke_quality_review_dir is not None
        and not args.report_only
        and not package_results_command
    ):
        raise ValueError(
            "--download-smoke-quality-review-dir is only supported with "
            "--report-only or package-results."
        )
