...
--enable-ncbi-discovery --email USER`. Fields:
...
column accepts yes/no-style boolean values. Without `--dry-run`,
...
`Source Audit Summary` with audit-status counts and a review note when mismatch
or manual-review rows are present. Fields:
