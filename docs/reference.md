...
- `verify-genus` and `verify-release-genus`: compact JSON summary with command,
  ...
  `download-smoke prepare --write --outdir <isolated-bounded-download-smoke-dir>`),
  ...
  promotion. Top-level `next_actions` also includes a
  `selection_review_strategy` entry so AI controllers that inspect only the
...
- `selection_review_strategy.v1`: compact JSON stdout schema for `selection-review strategy`. Stable fields include `schema_version`, `command`, `status`, `reason`, `outdir`, `selection_review_required`, `selected_row_count`, `manifest_row_count`, `bounded_smoke_selected_row_count`, `high_quality_planned_row_count`, `draft_or_fragmented_planned_row_count`, `unknown_assembly_level_planned_row_count`, `planned_refseq_category_counts`, `recommended_strategy`, `recommended_quality_tier`, `first_round_limit`, `bounded_smoke_outdir`, `selected_accession_quality_preview`, `selected_accession_quality_preview_truncated`, `selected_datasets_command_preview`, `selected_datasets_command_preview_truncated`, `selected_datasets_command_preview_only`, `review_artifacts`, `recommended_commands`, `handoff_checklist`, `review_guidance`, `forbidden_without_explicit_approval`, `downloads_triggered`, `providers_contacted`, `network_access`, `external_tools`, `manifest_mutated`, `strict_upgrade_applied`, and `accepted_for_final_use`. The command reads only existing `verify-genus` run artifacts and is intended for operator or AI-controller strategy review after the default selection checkpoint. Optional `--bounded-smoke-outdir <dir>` only fills in the later `download-smoke prepare --write --outdir ...` handoff command; the strategy command still writes nothing. `selected_datasets_command_preview` is preview-only and `handoff_checklist` marks actual bounded `datasets` execution and final genome acceptance as separate approval steps. It does not write files, run `datasets`, inspect ZIP/FASTA payloads, contact providers, decide final genome usability, mutate workflow outputs, or change strict scientific status.
