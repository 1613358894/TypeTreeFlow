...
- `bounded_download_smoke_inspection.tsv`: local inspection rows for a bounded smoke plan after a separately authorized download attempt. Fields are `record_id`, `assembly_accession`, `zip_path`, `zip_exists`, `zip_valid`, `genome_fasta_present`, `genome_fasta_member_count`, `fasta_record_count`, `fasta_total_bases`, `fasta_longest_record_bases`, `fasta_n50_bases`, `fasta_ambiguous_bases`, `fasta_header_wgs_keyword_count`, `fasta_header_scaffold_keyword_count`, `fasta_header_contig_keyword_count`, `fasta_fragmentation_signal`, and `status`. The FASTA statistics, N50 value, header keyword counts, and fragmentation signal are counts-only audit visibility; raw sequence text and raw FASTA headers are not copied into the inspection files.
