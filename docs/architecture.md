...
`acquisition-worklist`, and `count-crosswalk` are dispatched before full
workflow configuration is loaded. They emit compact JSON for AI operators and
remain outside workflow mutation, provider access, downloads, and external-tool
execution.
