...
def _write_external_genomes_install_plan_triplet(directory: Path) -> None:
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "external_genome_install_plan_summary.json").write_text(
        json.dumps(
            {
                "schema_version": "external_genomes_install_plan.v1",
                "status": "pass",
                "command": "coverage-pipeline external-genomes-install-plan",
                "target_outdir": "future-registration-run",
                "record_count": 2,
                "valid_count": 2,
                "invalid_count": 0,
                "registration_status_counts": {"external_genome_registered": 2},
                "install_plan_count": 2,
                "install_planned_count": 2,
                "install_skipped_count": 0,
                "install_plan_status_counts": {
                    "external_genome_install_planned": 2
                },
                "diagnostic_count": 0,
                "diagnostics": [],
                "audit_only": True,
                ...
                "summary": "Coverage pipeline external-genomes install plan passed",
            }
        ),
        encoding="utf-8",
    )
    registration_rows = [
        {
            ...
            "status": "external_genome_registered",
            "valid": "true",
            "message": "private message must not render",
            "notes": "private notes must not render",
        },
        {
            "species": "Clostridium beta",
    ...
    with (directory / "external_genome_registration_results.tsv").open(
        "w", encoding="utf-8", newline=""
    ) as handle:
        writer = csv.DictWriter(
            handle,
    ...
    install_rows = [
        {
            ...
            "external_genome_id": "DSM-1",
            "external_source_url": "https://example.invalid/provider/private",
            "source_genome_fasta_path": r"D:\private\provider\local.fna",
            "installed_genome_path": r"D:\private\run\genomes\DSM-1.fna",
            "sha256": "a" * 64,
            "is_type_material": "true",
        ...
        },
        {
            ...
            "external_genome_id": "DSM-2",
            "external_source_url": "https://example.invalid/provider/private-2",
            "source_genome_fasta_path": r"D:\private\provider\local2.fna",
            "installed_genome_path": r"D:\private\run\genomes\DSM-2.fna",
            "sha256": "b" * 64,
            "is_type_material": "true",
            "status": "external_genome_install_planned",
            "notes": "private notes must not render",
    ...
    with (directory / "external_genome_install_plan.tsv").open(
        "w", encoding="utf-8", newline=""
    ) as handle:
        writer = csv.DictWriter(
            handle,
...
def test_external_genomes_install_plan_section_is_bounded_and_audit_only(
    ...
    install_dir = tmp_path / "external-genomes-install-plan"
    _write_external_genomes_install_plan_triplet(install_dir)

    markdown = build_run_summary_markdown(
        [_record("ref1")],
        get_output_paths(tmp_path / "run"),
        SimpleNamespace(
            coverage_pipeline_dir=tmp_path,
            external_genomes_install_plan_dir=install_dir,
        ),
    )

    assert "## External Genomes Install Plan Audit" in markdown
    assert (
        "- Counts: record_count=2; valid_count=2; invalid_count=0; "
        "install_plan_count=2; install_planned_count=2; install_skipped_count=0; "
        "diagnostic_count=0; downloads_triggered=0; providers_contacted=0; "
        "network_access=false; manifest_mutated=false; "
        "writes_workflow_outputs=false; install_executed=false; "
        "external_genomes_registration_applied=false; "
        "target_outdir_mutated=false; audit_only=true; "
        "strict_scientific_deliverable=false"
    ) in markdown
    assert "does not copy FASTA files" in markdown
    assert "| external_genome_registered | 2 |" in markdown
    assert "| external_genome_install_planned | 2 |" in markdown
    assert "private provider" not in markdown
    assert "private message" not in markdown
    assert "private notes" not in markdown
    assert "DSM-1.fna" not in markdown


def test_external_genomes_install_plan_reader_warns_on_partial_malformed(
    tmp_path,
):
    install_dir = tmp_path / "external-genomes-install-plan"
    install_dir.mkdir()
    ...
    audit = read_optional_external_genomes_install_plan_audit(install_dir)
