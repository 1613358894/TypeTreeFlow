...
def _expected_download_smoke_inspection_result_defaults():
    return {
        ...
        "download_smoke_inspection_multiple_genome_fasta_members_count": 0,
...
def test_coverage_pipeline_server_validation_result_validate_accepts_valid_json(
    ...
    assert payload["download_smoke_inspection_multiple_genome_fasta_members_count"] == 1
    ...
    assert (
        payload["download_smoke_inspection_quality_gate_recommendation"]
        == "rerun_with_fragmentation_quality_gates"
    )
    assert payload[
        "download_smoke_inspection_quality_gate_recommendation_reasons"
...
def test_coverage_pipeline_server_validation_result_blocks_invalid_metadata(
    ...
    result["download_smoke_inspection_quality_gate_recommendation"] = [
        "not string"
    ]
    result["download_smoke_inspection_quality_gate_recommendation_reasons"] = [
        "ok",
        "",
    ]
    result_path = tmp_path / "coverage_handoff_server_validation_result.json"
    ...
    assert payload["download_smoke_inspection_fasta_quality_gate_blocker_counts"] == {}
    assert payload["download_smoke_inspection_quality_gate_recommendation"] == ""
    assert payload["download_smoke_inspection_quality_gate_recommendation_reasons"] == []
...
def test_coverage_pipeline_queue_operator_route_selects_first_matching_task(
    capsys, tmp_path
):
    checklist, reconciler, gaps, archive = _write_inputs(tmp_path)
    ...
    assert captured.err == ""
    assert payload["selected_coverage_queue_item_id"] == ""
    assert payload["selected_coverage_queue_operator_route"] == "provider_handoff"
    assert payload["selected_coverage_queue_item_found"] is True
    assert payload["coverage_next_task_packet"]["queue_item_id"] == (
        "cq004_prepare_provider_handoff"
    )
    assert payload["coverage_next_task_packet"]["operator_route"] == (
        "provider_handoff"
    )
    assert payload["coverage_next_command_plan"]["target_argv"] == [
        "provider-handoff",
    ...
    assert code == 0
    assert captured.err == ""
    assert status_payload["selected_coverage_queue_operator_route"] == (
        "provider_handoff"
    )
    assert status_payload["coverage_next_task_packet"]["queue_item_id"] == (
        "cq004_prepare_provider_handoff"
...
def test_coverage_pipeline_queue_operator_route_fails_closed(
    capsys, tmp_path
):
    checklist, reconciler, gaps, archive = _write_inputs(tmp_path)
    ...
    assert captured.err == ""
    assert payload["status"] == "blocked"
    assert payload["selected_coverage_queue_operator_route"] == "download_ready_ncbi"
    assert payload["selected_coverage_queue_item_found"] is False
    assert payload["coverage_next_task_packet"]["available"] is False
    assert payload["diagnostics"][-1]["component"] == "coverage_action_queue"
    assert payload["diagnostics"][-1]["diagnostic_code"] == (
        "queue_operator_route_not_found"
    )
    ...
    assert payload["selected_coverage_queue_operator_route"] == (
        "public_metadata_review"
    )
    assert payload["selected_coverage_queue_item_found"] is False
    assert payload["diagnostics"][-1]["component"] == "coverage_action_queue"
...
def test_coverage_pipeline_build_writes_isolated_outputs_and_force(capsys, tmp_path):
    ...
    result_template["download_smoke_inspection_quality_gate_recommendation"] = (
        "rerun_with_fragmentation_quality_gates"
    )
    ...
    assert (
        result_artifact["download_smoke_inspection_quality_gate_recommendation"]
        == "rerun_with_fragmentation_quality_gates"
    )
    assert result_artifact[
        "download_smoke_inspection_quality_gate_recommendation_reasons"
    ...
    assert (
        result_status_parent[
            "handoff_server_validation_download_smoke_inspection_quality_gate_recommendation"
        ]
        == "rerun_with_fragmentation_quality_gates"
    )
    assert result_status_parent[
        "handoff_server_validation_download_smoke_inspection_quality_gate_recommendation_reasons"
    ] == [
        "fragmented_fasta_signal_observed",
        "fasta_header_fragment_keywords_observed",
    ]
